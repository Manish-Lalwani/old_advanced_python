...
class EmailFetcher:
    def __init__(self,hostname="outlook.office365.com",username="wirelineproject@pi108.com",password="RHryLbzux6VjRWFw"):
        ...
        self.imap_obj2: Optional[ImaplibImap4SSL] = None
    ...
    def imap_object_init(self) -> ImaplibImap4SSL:
        ...
        self.imap_obj = imap_obj #for reading mail in readonly mode

        imap_obj2 = imaplib.IMAP4_SSL(self.hostname)
        imap_obj2.login(user=self.username, password=self.password)
        self.imap_obj2 = imap_obj2  # write mode #as if imap.select is readonly we cannot change state of the mail
    ...
    def get_email_reference(self,mailbox="inbox",readonly=False,charset=None,section='UNSEEN',sent_since="01-Apr-2021",subject="Kindly") -> List[bytes]:
        """FETCHES EMAIL REFERENCE NUMBER AND RETURNS IT"""
        self.imap_obj.select(mailbox=mailbox, readonly=readonly)
        self.imap_obj2.select(mailbox=mailbox, readonly=True)
    ...
    def get_emails_details_and_attachments(self,email_obj_list,email_ref,parent_output_dir="./attachment_files"):
        ...
        try:
            ...
            for i,mail in enumerate(email_obj_list):
                ...
                self.imap_obj2.uid('STORE', email_ref[i], '+FLAGS', '\SEEN')    #  marking the mail as seen
...
if __name__ == "__main__":
    ...
    email_obj_list = ef.get_email_objects(email_references_bytes_list=email_reference_list[:2])
    print(email_obj_list[0]["from"])
    email_detail_list =ef.get_emails_details_and_attachments(email_obj_list,email_ref=email_reference_list[:2])
